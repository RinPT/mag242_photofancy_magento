...
namespace Customweb\DatatransCw\Plugin\Sales\Controller\Adminhtml\Order\Create;
...
class Save
{
	...
	public function aroundExecute(\Magento\Sales\Controller\Adminhtml\Order\Create\Save $subject, \Closure $proceed)
	{
		...
		$this->_coreRegistry->register('datatranscwcheckout_moto', true);
		$result = $proceed();
		if ($this->_messageManager->getMessages()->getLastAddedMessage() != null
			...
			$order = $this->_coreRegistry->registry('datatranscw_checkout_last_order');
			if ($order instanceof \Magento\Sales\Model\Order) {
				if ($order->getPayment()->getMethodInstance() instanceof \Customweb\DatatransCw\Model\Payment\Method\AbstractMethod) {
					...
					return $this->_redirectResultFactory->create()->setPath('datatranscw/checkout/payment', ['order_id' => $order->getId(), 'send_confirmation' => $sendConfirmation]);
