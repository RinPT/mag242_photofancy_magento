...
define([
	...
	'Customweb_DatatransCw/js/authorizationMethod',
	'Customweb_DatatransCw/js/alias'
], function(
	$,
	Component,
	mageTemplate,
	...
	return Component.extend({
		defaults: {
			template: 'Customweb_DatatransCw/payment/datatranscw_byjunosingleinvoice',
			fieldErrorTemplate: '<div for="<%- id %>" generated="true" class="mage-error" id="<%- id %>-error"><%- message %></div>',
			authorizationUrl: url.build('datatranscw/checkout/authorize/'),
			restoreCartUrl: url.build('datatranscw/checkout/restoreCart/')
		},
		...
		getForm: function() {
			...
			if (!customer.isLoggedIn()) {
				serviceUrl = urlBuilder.createUrl('/guest-carts/:cartId/datatranscw/checkout/payment-form/:paymentMethod', {
					cartId: quote.getQuoteId(),
					paymentMethod: this.item.method
				});
			} else {
				serviceUrl = urlBuilder.createUrl('/carts/mine/datatranscw/checkout/payment-form/:paymentMethod', {
					paymentMethod: this.item.method
				});
			}
			storage.get(
