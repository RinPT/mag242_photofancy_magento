 ...
 *
 */
class Customweb_Datatrans_BackendOperation_Adapter_CaptureAdapter extends Customweb_Datatrans_AbstractMaintenanceAdapter implements Customweb_Payment_BackendOperation_Adapter_Service_ICapture {
	...
	public function partialCapture(Customweb_Payment_Authorization_ITransaction $transaction, $items, $close){
		...
		$xmlRequestBuilder = new Customweb_Datatrans_BackendOperation_Adapter_CaptureRequestBuilder($transaction, $this->getConfiguration(), $amount);
		...
		$transaction->partialCapture($amount, $close);
	}
}